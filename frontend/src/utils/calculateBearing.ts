...
export const calculateBearing = (params: {
  from: LatLng;
  to: LatLng;
}) => {
  ...
  const y = Math.cos(degreesToRadians(from.lat)) * Math.sin(degreesToRadians(to.lat)) - (Math.sin(degreesToRadians(from.lat)) * Math.cos(degreesToRadians(to.lat)) * Math.cos(deltaLon));
