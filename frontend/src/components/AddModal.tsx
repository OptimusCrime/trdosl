      ...
      <div className="flex flex-col space-y-4 pt-2">
          ...
          <div className="flex flex-row justify-between items-center max-w-[300px]">
            ...
            >
              {Array.from(Array(100).keys()).map(value => (
                <option key={`hours_${value}`} value={addLeadingZero(value)}>{addLeadingZero(value)}</option>
              ))}
            </select>
            ...
            >
              {Array.from(Array(60).keys()).map(value => (
                <option key={`minutes_${value}`} value={addLeadingZero(value)}>{addLeadingZero(value)}</option>
              ))}
            </select>
            <div className="flex">:</div>
            <select
            ...
            >
              {Array.from(Array(60).keys()).map(value => (
                <option key={`seconds_${value}`} value={addLeadingZero(value)}>{addLeadingZero(value)}</option>
              ))}
            </select>
          </div>
        </div>
        <div className="form-control w-full">
          ...
          <div className="flex flex-row justify-between items-center max-w-[187px]">
            ...
            >
              {Array.from(Array(100).keys()).map(value => (
                <option key={`minutes_${value}`} value={value}>{addLeadingZero(value)}</option>
              ))}
            </select>
          </div>
        </div>
